...
import (
	...
	"gbchain-org/go-gbchain/accounts/scwallet"
	"gbchain-org/go-gbchain/accounts/usbwallet"
...
func (c *Config) NodeName() string {
	...
	if name == "gbchian" || name == "gbchian-testnet" {
		name = "GBChian"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
func (c *Config) ResolvePath(path string) string {
	...
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'gbchian' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-GBChain-keystore") //Account dir
		ephemeral = keydir
	}
