...
var (
	...
	FrontierBlockReward       = big.NewInt(1.2168e+18) // Block reward in wei for successfully mining a block
	ByzantiumBlockReward      = big.NewInt(0.5e+18)    // Block reward in 3wei for successfully mining a block upward from Byzantium
	ConstantinopleBlockReward = big.NewInt(0.5e+18)    // Block reward in 2wei for successfully mining a block upward from Constantinople
...
// the difficulty that a new block should have when created at time
// given the parent block's time and difficulty.
//难度系数调整算法:根据父块的时间和难度，它返回创建新块时应该具有的难度。
//time 新区块的生成时间
//parent 上一个区块的参数
