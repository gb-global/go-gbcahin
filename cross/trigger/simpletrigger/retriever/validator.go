...
func NewCreditleValidator(contract common.Address, chain simpletrigger.BlockChain, config *cross.Config, chainConfig *params.ChainConfig) *CreditValidator {
...
func (v *CreditValidator) IsLocalCtx(ctx trigger.Transaction) bool {
	return v.chainConfig.ChainID.Cmp(ctx.ChainId()) == 0
}

func (v *CreditValidator) IsRemoteCtx(ctx trigger.Transaction) bool {
	return v.chainConfig.ChainID.Cmp(ctx.DestinationId()) == 0
}

func (v *CreditValidator) RequireSignatures() int {
	return v.requireSignature
}

func (v *CreditValidator) ExpireNumber() int {
	return expireNumber
}

func (v *CreditValidator) VerifyExpire(ctx *cc.CrossTransaction) error {
	// discard if expired
 ...
 */
func (v *CreditValidator) VerifySigner(ctx *cc.CrossTransaction, signChain, validChain *big.Int) (common.Address, error) {
...
func (v *CreditValidator) VerifyContract(cws trigger.Transaction) error {
...
func (v *CreditValidator) UpdateAnchors(info *cc.RemoteChainInfo) error {
	v.mu.Lock()
	defer v.mu.Unlock()
	newHead := v.chain.CurrentBlock().Header() // Special case during testing
	statedb, err := v.chain.StateAt(newHead.Root)
