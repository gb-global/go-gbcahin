...
import (
	...
	"gbchain-org/go-gbchain/common/fdlimit"
	"gbchain-org/go-gbchain/consensus"
	"gbchain-org/go-gbchain/consensus/clique"
	"gbchain-org/go-gbchain/consensus/ethash"
	"gbchain-org/go-gbchain/consensus/scrypt"
	...
	"gbchain-org/go-gbchain/ethstats"
	"gbchain-org/go-gbchain/graphql"
	"gbchain-org/go-gbchain/les"
	"gbchain-org/go-gbchain/log"
	"gbchain-org/go-gbchain/metrics"
	"gbchain-org/go-gbchain/metrics/influxdb"
	"gbchain-org/go-gbchain/miner"
	"gbchain-org/go-gbchain/node"
	"gbchain-org/go-gbchain/p2p"
	"gbchain-org/go-gbchain/p2p/discv5"
	...
	whisper "gbchain-org/go-gbchain/whisper/whisperv6"
...
var (
	...
	StratumHashRate = cli.BoolFlag{
		...
		Usage: "calc stratum miner's hashRate , if turn on,gbchian can estimate stratum miner's HashRate",
	}
	...
	SUBRPCEnabledFlag = cli.BoolFlag{
		Name:  "sub.rpc",
		Usage: "Enable the HTTP-RPC server for gbchain",
	}
	SUBRPCListenAddrFlag = cli.StringFlag{
		Name:  "sub.rpcaddr",
		Usage: "HTTP-RPC server listening interface for gbchain",
		Value: node.DefaultSubHTTPHost,
	}
	SUBRPCPortFlag = cli.IntFlag{
		Name:  "sub.rpcport",
		Usage: "HTTP-RPC server listening port for gbchain",
		Value: node.DefaultSubHTTPPort,
	}
	SUBRPCApiFlag = cli.StringFlag{
		Name:  "sub.rpcapi",
		Usage: "API's offered over the HTTP-RPC interface for gbchain",
		Value: "",
	}
	SUBIPCDisabledFlag = cli.BoolFlag{
		Name:  "sub.ipcdisable",
		Usage: "Disable the IPC-RPC server for gbchain",
	}
	SUBIPCPathFlag = DirectoryFlag{
		Name:  "sub.ipcpath",
		Usage: "Filename for IPC socket/pipe within the datadir (explicit paths escape it) for gbchain",
	}
	SUBWSEnabledFlag = cli.BoolFlag{
		Name:  "sub.ws",
		Usage: "Enable the WS-RPC server for gbchain",
	}
	SUBWSListenAddrFlag = cli.StringFlag{
		Name:  "sub.wsaddr",
		Usage: "WS-RPC server listening interface for gbchain",
		Value: node.DefaultSubWSHost,
	}
	SUBWSPortFlag = cli.IntFlag{
		Name:  "sub.wsport",
		Usage: "WS-RPC server listening port for gbchain",
		Value: node.DefaultSubWSPort,
	}
	SUBWSApiFlag = cli.StringFlag{
		Name:  "sub.wsapi",
		Usage: "API's offered over the WS-RPC interface for gbchain",
		Value: "",
	}
	SUBWSAllowedOriginsFlag = cli.StringFlag{
		Name:  "sub.wsorigins",
		Usage: "Origins from which to accept websockets requests for gbchain",
		Value: "",
	}
	SUBRPCCORSDomainFlag = cli.StringFlag{
		Name:  "sub.rpccorsdomain",
		Usage: "Comma separated list of domains from which to accept cross origin requests (browser enforced) for gbchain",
		Value: "",
	}
	SUBRPCVirtualHostsFlag = cli.StringFlag{
		Name:  "sub.rpcvhosts",
		Usage: "Comma separated list of virtual hostnames from which to accept requests (server enforced). Accepts '*' wildcard.for gbchain",
		Value: strings.Join(node.DefaultConfig.HTTPVirtualHosts, ","),
	}
	...
	RoleFlag = TextMarshalerFlag{
		Name:  "role",
		Usage: "it can be one of (mainchain,gbchain,anchor)",
		Value: &role,
	}
...
func RegisterEthService(stack *node.Node, cfg *eth.Config) <-chan *sub.Ethereum {
	...
	if cfg.SyncMode == downloader.LightSync {
		...
		if cfg.Role.IsMainChain() {
			...
			if err != nil {
				Fatalf("Failed to register the gbchain service: %v", err)
				return nil
			}
