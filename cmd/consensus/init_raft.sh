...
for i in $(seq 1 "${numNodes}"); do
  mkdir -p $dir/dd"${i}"/{keystore,gbchian}
  cp $dir/nodekey/static-nodes.json $dir/dd"${i}"/static-nodes.json
  cp $dir/nodekey/nodekey"${i}" $dir/dd"${i}"/gbchian/nodekey
  cp $dir/nodekey/keys/key"${i}" $dir/dd"${i}"/keystore/key"${i}"
  gbchian --datadir $dir/dd"${i}" init $dir/nodekey/genesis_raft.json --role=subchain
done
